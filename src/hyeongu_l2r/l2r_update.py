...
class L2RUpdate(object):
    ...
    def replace(self, buffer):
        ...
        for qid, candidate_docids in tqdm(
            self.candidate_neg_docids.items(), total=len(self.candidate_neg_docids)
        ):
            ...
            if place_left == 0:  # buffer满， 执行替换
                mem_eval_size = min(
                    self.mem_eval_size, len(buffer.buffer_qid2dids[qid])
                )
                mem_eval_docids_lst, mem_eval_indices = random_retrieve(
                    buffer.buffer_qid2dids[qid], mem_eval_size, return_indices=True
                )
                # mem_eval_model_out = self.get_model_out(buffer, model_temp, qid, mem_eval_docids_lst)  # [mem_eval_size, 768]

                mem_upsample_num = min(
                    len(buffer.buffer_qid2dids[qid]) - mem_eval_size,
                    int(self.mem_replace_size * self.upsample_scale),
                )  # 上采样mem数据个数
                upsample_mem_docids_lst, upsample_mem_indices = random_retrieve(
                    buffer.buffer_qid2dids[qid],
                    mem_upsample_num,
                    excl_indices=mem_eval_indices,
                    return_indices=True,
                )
                # mem_model_out = self.get_model_out(buffer, model_temp, qid, upsample_mem_docids_lst)

                new_upsample_num = min(
                    len(candidate_docids),
                    int(self.mem_replace_size * self.upsample_scale),
                )  # 上采样新数据个数
                upsample_candidate_docids_lst = random_retrieve(
                    candidate_docids, new_upsample_num
                )
                # new_model_out = self.get_model_out(buffer, model_temp, qid, upsample_candidate_docids_lst)

                if self.params.compatible:
                    mem_eval_model_out = torch.tensor(
                        np.array(
                            [
                                buffer.buffer_did2emb[int(docid)]
                                for docid in mem_eval_docids_lst
                            ]
                        ),
                        device=self.train_params.device,
                    )
                    mem_model_out = torch.tensor(
                        np.array(
                            [
                                buffer.buffer_did2emb[int(docid)]
                                for docid in upsample_mem_docids_lst
                            ]
                        ),
                        device=self.train_params.device,
                    )
                    new_model_out = self.get_model_out(
                        buffer, model_temp, qid, upsample_candidate_docids_lst
                    )
                else:
                    model_out = self.get_model_out(
                        ...
                        qid,
                        mem_eval_docids_lst
                        + upsample_mem_docids_lst
                        + upsample_candidate_docids_lst,
                    )
                    mem_eval_model_out = model_out[
                        :mem_eval_size, :
                    ]  # [mem_eval_size, 768]
                    mem_model_out = model_out[
                        mem_eval_size : mem_eval_size + mem_upsample_num, :
                    ]
                ...
                new_sim = cosine_similarity(
                    new_model_out, mem_eval_model_out
                )  # [new_upsample_num, mem_eval_size]
                new_sim = (
                    torch.sum(new_sim, dim=-1) * (-1.0) / new_sim.size(-1)
                )  # 选相似度最小的一些
                new_indices = new_sim.sort(dim=0, descending=True)[1]

                mem_replace_size = min(
                    [self.mem_replace_size, len(mem_sim), len(new_sim)]
                )
                indices = indices[:mem_replace_size]
                new_indices = new_indices[:mem_replace_size]
                # 执行替换
                buffer.buffer_qid2dids[qid] = np.array(buffer.buffer_qid2dids[qid], dtype=object) # 수정
                ######
                candidate_dids = np.array(upsample_candidate_docids_lst)[new_indices.cpu()]
                candidate_dids = candidate_dids.tolist() 
                update_indices = np.array(upsample_mem_indices)[indices.cpu()]
                for i, idx in enumerate(update_indices):
                    buffer.buffer_qid2dids[qid][idx] = candidate_dids[i]
                
                # buffer.buffer_qid2dids[qid][
                #     np.array(upsample_mem_indices)[indices.cpu()]
                # ] = candidate_dids
                #####
                # buffer.buffer_qid2dids[qid][
                #     np.array(upsample_mem_indices)[indices.cpu()]
                # ] = np.array(upsample_candidate_docids_lst)[new_indices.cpu()].copy()
                ...
                indices = list(
                    np.random.choice(all_indices, num_retrieve, replace=False)
                )
                ...
                mem_eval_docids_lst = random_retrieve(
                    buffer.buffer_qid2dids[qid], mem_eval_size
                )

                upsample_num = min(
                    len(candidate_docids), int(place_left * self.upsample_scale)
                )  # 两倍量的采样
                upsample_candidate_docids_lst = random_retrieve(
                    candidate_docids, upsample_num
                )  # list
                ...
                if self.params.compatible:
                    ...
                    candidate_model_out = self.get_model_out(
                        buffer, model_temp, qid, upsample_candidate_docids_lst
                    )  # [upsample_num, 768]
                else:
                    model_out = self.get_model_out(
                        ...
                        model_temp,
                        qid,
                        mem_eval_docids_lst + upsample_candidate_docids_lst,
                    )  # [mem_eval_size+upsample_num, 768]
                    mem_eval_model_out = model_out[:mem_eval_size, :]
                    candidate_model_out = model_out[mem_eval_size:, :]

                inter_sim = cosine_similarity(
                    candidate_model_out, mem_eval_model_out
                )  # [upsample_num, mem_eval_size]
                inter_sim = (
                    torch.sum(inter_sim, dim=-1) * (-1.0) / inter_sim.size(-1)
                )  # [upsample_num] 选相似度最小的一些

                num_new = min(place_left, len(candidate_model_out))
                indices = inter_sim.sort(dim=0, descending=True)[1][:num_new]  ##수정함
                selected_docids = np.array(upsample_candidate_docids_lst, dtype=object)[indices.cpu()].tolist()
                buffer.buffer_qid2dids[qid].extend(selected_docids)
                # buffer.buffer_qid2dids[qid].extend(
                #     np.array(upsample_candidate_docids_lst,dtype=object)[indices.cpu()]
                # )

    def get_model_out(self, buffer, model_temp, qid, docids_lst):
        # if len(docids_lst)==0:
        #     print(f"qid:{qid}, docids_lst:{docids_lst}")
        if any(isinstance(x, str) and len(x) <= 1 for x in docids_lst):
            print('docids_lst:',docids_lst)
